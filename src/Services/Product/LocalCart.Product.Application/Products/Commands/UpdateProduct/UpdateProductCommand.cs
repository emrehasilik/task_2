...
namespace LocalCart.Product.Application.Products.Commands.UpdateProduct;
...
public sealed class UpdateProductCommandValidator : AbstractValidator<UpdateProductCommand>
{
    public UpdateProductCommandValidator()
    {
        ...
        RuleFor(command => command.StockQuantity).InclusiveBetween(0, 1_000_000);
...
public sealed class UpdateProductCommandHandler(
    ...
    IDateTimeProvider clock) : IRequestHandler<UpdateProductCommand, ProductResponse>
{
    public async Task<ProductResponse> Handle(UpdateProductCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await products.SlugExistsAsync(normalizedSlug, product.Id, cancellationToken).ConfigureAwait(false))
        {
